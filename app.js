(() => {
  ...
  const groupById = (id) =>
    DATA.groups.find(g => g.id === Number(id));

  const selectedGroup = () =>
    groupById(state.group);
  ...
  function loadSelection() {
    try {
      ...
      if (!g || !g.sections.includes(Number(saved.section))) {
        return;
      }
  ...
  function renderGroups() {
    ...
    DATA.groups.forEach(group => {
      ...
      button.setAttribute(
        'aria-pressed',
        String(state.group === group.id)
      );

      button.addEventListener('click', () => {
        state.group = group.id;

        state.section = group.sections.includes(state.section)
          ? state.section
          : null;
        ...
        selectionStatus.textContent =
          ...
          `${group.sections[0]}–${group.sections[group.sections.length - 1]}.`;
      });
  ...
  function renderSections() {
    ...
    sectionHelp.textContent =
      `Sections available for ${group.label}: ` +
      `${group.sections[0]}–${group.sections[group.sections.length - 1]}.`;

    for (let i = 1; i <= 15; i++) {
      addSectionButton(
        i,
        !group.sections.includes(i)
      );
    }
  }
  ...
  function addSectionButton(section, disabled) {
    ...
    button.className =
      'option-btn' +
      (state.section === section ? ' section-selected' : '');
  ...
  function appliesToSection(session, sectionId) {
    return session.sections == null ||
      (
        ...
        (
          session.sections.includes('ALL') ||
          session.sections.includes(sectionId)
        )
      );
  }
  ...
  function filteredSessions() {
    ...
    return DATA.sessions
      ...
      .sort(
        (a, b) =>
          DATA.metadata.days.indexOf(a.day) -
          DATA.metadata.days.indexOf(b.day) ||
          DATA.metadata.timeSlots.indexOf(a.timeSlot) -
          DATA.metadata.timeSlots.indexOf(b.timeSlot)
      );
  }

  function setTheme() {
    timetableScreen.classList.remove(
      'group-1',
      'group-2',
      'group-3'
    );

    timetableScreen.classList.add(
      `group-${state.group}`
    );
  ...
  function renderTimetable() {
    ...
    timetableGrid.classList.toggle(
      'hidden',
      sessions.length === 0
    );

    mobileTimetable.classList.toggle(
      'hidden',
      sessions.length === 0
    );
  ...
  function sessionCard(session) {
    ...
    card.className =
      `session-card ${isSection ? 'section' : 'lecture'}`;
    ...
    if (isSection) {
      ...
      if (
        session.instructor &&
        session.instructor !== 'Not specified in PDF'
      ) {
        addMeta(
          meta,
          'Instructor',
  ...
  downloadBtn.addEventListener('click', () => {
    ...
    // PDF files are stored in the repository root.
    link.href =
      `G${state.group}_S${state.section}.pdf`;

    link.download =
      `MNU_Level4_Group${state.group}_Section${state.section}_Timetable.pdf`;
  ...
  emptyChangeBtn.addEventListener(
    'click',
    showSelection
  );
  ...
  if (state.group && state.section) {
    selectionStatus.textContent =
      `Saved selection: Group ${state.group} + Section ${state.section}.`;
  }
})();
